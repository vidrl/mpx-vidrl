...
process DepleteAligned {

    label "mgp_tools"
    tag { "$id : $idx_name" }

    publishDir "$params.outdir/$params.stage/$params.subdir", mode: "copy", pattern: "${id}_${idx_name}.json"
    publishDir "$params.outdir/$params.stage/$params.subdir/fastq", mode: "symlink", pattern: "${id}_${idx_name}_depleted*.fastq"  // always to fastq sub-subdir
    ...
    if (forward.size() > 0 && reverse.size() > 0) // guard against Needletail error on empty file
        ...
        mgp-tools deplete \
            ...
            --input $forward \
            --input $reverse \
            --output ${id}_${idx_name}_depleted_1.fastq \
            --output ${id}_${idx_name}_depleted_2.fastq \
